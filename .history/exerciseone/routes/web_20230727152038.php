...
Route::get('books/read',function(){
    ...
    // $results = DB::table('books')->selectRaw(DB::raw('title,description'))->where('id',1)->get();   // should use DB::raw() only when you need to include raw SQL expressions that cannot be handled directly by Laravel's Query Builder.
    should use DB::raw() only when you need to include raw SQL expressions that cannot be handled directly by Laravel's Query Builder.
